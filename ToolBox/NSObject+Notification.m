...
- (void)setObserverDict:(NSMutableDictionary *)observerDict {
    objc_setAssociatedObject(self, @selector(observerDict), observerDict, OBJC_ASSOCIATION_RETAIN);
}

- (NSMutableDictionary *)observerDict {
    return objc_getAssociatedObject(self, @selector(observerDict));
}
